...
class VideoCell: UITableViewCell, RegisterCellFromNib {
    ...
    var video = NewsModel() {
        didSet {
            ...
            if let urlString = video.video_detail_info.detail_video_large_image.urlString, !urlString.isEmpty {
                bgImageButton.kf.setBackgroundImage(with: URL(string: urlString)!, for: .normal)
            }
